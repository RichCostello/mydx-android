...
include($_SERVER['DOCUMENT_ROOT'] . '/db/mysql.lib.php');
    ...
    $arrQStr=explode("&", $encryptme->decode($_SERVER['QUERY_STRING']));
    $arrval1=explode("=",$arrQStr[0]);
    $arrval2=explode("=",$arrQStr[1]);
    $arrval3=explode("=",$arrQStr[2]);
    $arrval4=explode("=",$arrQStr[3]);
    $arrval5=explode("=",$arrQStr[4]);
    
    $strain_id=$arrval5[1];
    
    $profileid=$arrval1[1];
    
    $graphval=urldecode($arrval2[1]);
    ...
    while($row = mysql_fetch_array($result))
    {
		...
		$straincontent.= '<div class="form-content"> <div class="col-xs-offset-1 col-xs-8 "><h5>'.$row["Name"].'</h5></div>';
        
		$straincontent.= '<div class="col-xs-2 form-content">'. $row["Value"] . '</div><label for="percent" class="col-xs-1 clabel-details">'. $row['Unit']  .'</label> </div>';
...
function showGraph(){
    ...
    r.barchart(10, 10, 205, 200, [[<?php echo $graphval ?>]], 0, {type: "sharp"}).attr({fill: "#ff0000"});
};
</script>  </head>
...
<option value="120">>2 hrs</option>
 ...
 <p class="sharel text-center"><a  href="javascript:fbShareGs('<?=$fbSURL?>', 'Fb Share', 'MyDx - A Device for a better life', '', 520, 350)">Facebook</a></p>
