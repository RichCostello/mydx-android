...
(function() {
    function B(f, m, i, b, a, h, n, A) {
        ...
        switch (h) {
            case "round":
                a ? (a = ~~(i / 2), c = b < a ? ["M", f - ~~(i / 2), m,
                    "l", 0, 0, "a", ~~(i / 2), b, 0, 0, 1, i, 0,
                    "l", 0, 0, "z"
                ] : ["M", f - a, m, "l", 0, a - b, "a", a, a, 0,
                    1, 1, i, 0, "l", 0, b - a, "z"
                ]) : (a = ~~(b / 2), c = i < a ? ["M", f + 0.5, m +
                    0.5 - ~~(b / 2), "l", 0, 0, "a", i, ~~(b /
                        2), 0, 0, 1, 0, b, "l", 0, 0, "z"
                ] : ["M", f + 0.5, m + 0.5 - a, "l", i - a, 0,
                    "a", a,
                    a, 0, 1, 1, 0, b, "l", a - i, 0, "z"
                ]);
                break;
            case "sharp":
                a ? (a = ~~(i / 2), c = ["M", f + a, m, "l", -i, 0, 0, -
                    C(b - a, 0), a, -y(a, b), a, y(a, b), a,
                    "z"
                ]) : (a = ~~(b / 2), c = ["M", f, m + a, "l", 0, -b,
                    C(i - a, 0), 0, y(a, i), a, -y(a, i), a + (
                        2 * a < b), "z"
                ]);
                break;
            case "square":
                c = a ? ["M", f + ~~(i / 2), m, "l", 1 - i, 0, 0, -b, i -
                    1, 0, "z"
                ] : ["M", f, m + ~~(b / 2), "l", 0, -b, i, 0, 0, b,
                    "z"
                ];
                break;
            case "soft":
                a ? (a = y(Math.round(i / 5), b), c = ["M", f - ~~(i /
                        2), m, "l", 0, a - b, "a", a, a, 0, 0,
                    1, a, -a, "l", i - 2 * a, 0, "a", a, a, 0,
                    0, 1, a, a, "l", 0, b - a, "z"
                ]) : (a = y(i, Math.round(b / 5)), c = ["M", f + 0.5,
                    m + 0.5 - ~~(b / 2), "l", i - a,
                    0, "a", a, a, 0, 0, 1, a, a, "l", 0, b - 2 *
                    a, "a", a, a, 0, 0, 1, -a, a, "l", a - i, 0,
                    "z"
                ])
        }
    ...
    function E(f, m, i, b, a, h, n) {
        ...
        if (Raphael.is(h[0], "array")) {
            for (var w = [], l = q, q = 0, e = h.length; e--;) u.push(f
                    .set()), w.push(Math.max.apply(Math, h[e])), q =
                Math.max(q, h[e].length);
            if (n.stacked)
                for (e = q; e--;) {
        ...
        for (e = 0; e < q; e++) {
            ...
            for (d = 0; d < (l || 1); d++) {
                var j = Math.round((l ? h[d][e] : h[e]) * v),
                    x = i + a - r - j,
                    o = B(Math.round(k + b / 2), x + j, b, j, !0, A,
                        null, f).attr({
                        stroke: "none",
                        fill: y[l ? d : e]
                    });
            ...
            if (n.stacked) {
                t.push(d = f.rect(g[0].x - g[0].w / 2, i, b, a).attr(
                    this.shim));
                d.bars = f.set();
                ...
                for (var z = 0, D = g.length; z < D; z++) {
                    ...
                    o.h = j;
                    o.y = i + a - r - 0.5 * !!x - j;
                    p.push(j = f.rect(o.x - o.w / 2, o.y, b, o.value *
                        v).attr(this.shim));
                    j.bar = o;
                    j.value = o.value;
                    x += o.value
        ...
        if (!n.stacked)
            for (e = 0; e < q; e++) {
                for (d = 0; d < (l || 1); d++) p.push(j = f.rect(Math.round(
                        k), i + r, b, a - r).attr(this.shim)), j.bar =
                    l ? u[d][e] : u[e], j.value = j.bar.value, k += b;
                k += c
            }
                ...
                for (var c = 0; c < q; c++)
                    for (var m = 0, g = 0; g < (l || 1); g++) {
                        ...
                        if (g == l - 1) {
                            ...
                            d = f.text(u[c * (l || 1) + g].x, i + a -
                                r / 2, d).attr(txtattr).insertBefore(
                                p[c * (l || 1) + g]);
                            var k = d.getBBox();
                            if (k.x - 7 < j) d.remove();
                            else {
                        ...
                        for (c = 0; c < q; c++)
                            for (g = 0; g < (l || 1); g++) {
                                d = f.labelise(l ? b[g] && b[g][c] :
                                    b[c], l ? h[g][c] : h[c], w
                                );
                                d = f.text(u[c * (l || 1) + g].x, e ?
                                    i + a - r / 2 : u[c * (l ||
                                        1) + g].y - 10, d).attr(
                                    txtattr).insertBefore(p[c *
                                    (l || 1) + g]);
                                k = d.getBBox();
                                if (k.x - 7 < j) d.remove();
                                else {
    ...
    function F(f, m, i, b, a, h, n) {
        ...
        for (e = 0; e < q; e++) {
            ...
            for (d = 0; d < (l || 1); d++) {
                var v = l ? h[d][e] : h[e],
                    j = B(m, g + r / 2, Math.round(v * k), r - 1, !1, y,
                        null, f).attr({
                        stroke: "none",
                        fill: C[l ? d : e]
                    });
            ...
            if (n.stacked) {
                ...
                for (var o = 0, z = c.length; o < z; o++) {
                    ...
                    j.w = v;
                    j.x = m + v;
                    p.push(v = f.rect(m + x * k, j.y - j.h / 2, j.value *
                        k, r).attr(this.shim));
                    v.bar = j;
                    x += j.value
                }
        ...
        if (!n.stacked)
            for (e = 0; e < q; e++) {
                for (d = 0; d < (l || 1); d++) v = f.rect(m, g, b, r).attr(
                    this.shim), p.push(v), v.bar = l ? u[d][e] : u[
                    e], v.value = v.bar.value, g += r;
                g += a
            }
        s.label = function(a, b) {
            ...
            for (var c = 0; c < q; c++)
                for (var d = 0; d < l; d++) {
                    var e = f.labelise(l ? a[d] && a[d][c] : a[c],
                            l ? h[d][c] : h[c], w),
                        g = b ? "end" : "start";
                    this.labels.push(e = f.text(b ? u[c * (l || 1) +
                        d].x - r / 2 + 3 : m + 5, u[c *
                        (l || 1) + d].y, e).attr(txtattr).attr({
                        "text-anchor": g
                    }).insertBefore(p[0]));
                    e.getBBox().x < m + 5 ? e.attr({
                        x: m + 5,
                        "text-anchor": "start"
                    }) : u[c * (l || 1) + d].label = e
                }
